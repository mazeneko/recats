    ...
    <div>
      <label
        >Recast (minutes):<input
          type="number"
          [formField]="fields().recastMinutes"
    ...
    <div>
      <label
        >Recast (seconds):<input
          type="number"
          [formField]="fields().recastSeconds"
...
export function toDurationRecast(fieldsValue: DurationRecastFields): DurationRecast {
  ...
  const durationRecast = zodSafeParse(DurationRecast, {
    recastType: 'duration',
    recastDuration: Duration.ZERO.plusDays(fields.recastDays ?? 0)
...
/**
 * 時間によるリキャストのフィールドのデフォルト値を作成します。
 * @returns 時間によるリキャストのフィールドのデフォルト値
 */
