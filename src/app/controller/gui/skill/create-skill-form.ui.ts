...
import { apply, form, FormField, schema, submit } from '@angular/forms/signals';
...
@Component({
  ...
  template: `
    <form novalidate (submit)="$event.preventDefault(); emitCreateSkill()" class="flex flex-col">
      ...
      <div>
        <label
          >Initially Available:<input
            type="checkbox"
            [formField]="createSkillForm.initiallyAvailable"
        /></label>
      ...
      <div>
        <label
          >Charge Limit:<input
            type="number"
            [formField]="createSkillForm.castingChargeLimit"
